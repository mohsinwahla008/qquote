...
async function settingUpQtyFunc(driver){
  let priceQuantity4 = await driver.findElement(By.css("#quantity"));
    ...
    priceQuantity4.sendKeys(Key.BACK_SPACE,"4");
    await driver.sleep(3000)
}
...
async function upSellItemsFunc(driver){
  ...
  async function pfTaxesFunc(driver){
    ...
    let roundProDetails = Math.round(percentageWithProvisional*100)/100;
    let parsedProvisional =parseFloat(roundProDetails);
    console.log("Provisional Price is = ", parsedProvisional);
    let percentageWithFedral = parsedExSubTotal*parsedFedValue/100
    let roundFedDetails = Math.round(percentageWithFedral*100)/100;
    let parsedFedral =parseFloat(roundFedDetails);
...
async function upSellItemsFuncSaved(driver){
  ...
  async function pfSuperGrandSavedFunc(driver){
    ...
    let roundProSaved = Math.round(percentageProSaved*100)/100;
    let parsedFinalProSaved =parseFloat(roundProSaved);
    ...
    let roundFedSaved = Math.round(percentageFedSaved*100)/100;
    let parsedFinalFedSaved =parseFloat(roundFedSaved);
...
//First Name
const firstNameVQ = await driver.findElement(By.id("fname"));
await firstNameVQ.clear();
  await firstNameVQ.sendKeys("Mohsin")
  
  //Last Name
  const lastNameVQ = await driver.findElement(By.id("lname"));
  await lastNameVQ.clear();
...
//Third Screen
  ...
  await driver.sleep(4000)
//Updating Quantity in Sent Quote
let updateQty = await driver.findElement(By.xpath("/html[1]/body[1]/form[1]/div[2]/div[1]/div[1]/div[1]/div[1]/div[1]/div[2]/div[3]/p[1]/span[1]/input[1]"));
await driver.executeScript('arguments[0].scrollIntoView({ behavior: "smooth", block: "center", inline: "center" });', updateQty);
await updateQty.sendKeys(Key.CONTROL,'a')
await updateQty.sendKeys(Key.BACK_SPACE,'8');
...
async function upSellFuncSent(driver){
  ...
  async function pfGrandPriceSent(driver){
    ...
    let roundProSent = Math.round(percentageProSent*100)/100;
    let parsedFinalProSent =parseFloat(roundProSent);
    ...
    let roundFedSent = Math.round(percentageFedSent*100)/100;
    let parsedFinalFedSent =parseFloat(roundFedSent)
...
async function exportToExcel(dealershipName,tireSize,tireBrandFunc,partNumberFunc,unitQuantityFunc, totalPrice,
  ...
  ,pfGrandPriceSent,subtotal1Comparison,upSellPriceComparison,addOnsPriceComp ,
...
(async function example() {
  ...
  try {
    ...
    await exportToExcel(exportDealer,exportTireSize,exportTireBrand,
      ...
      ,exportAddOnsSent,exportSubtotal2Sent,exportGrandTotalSent
